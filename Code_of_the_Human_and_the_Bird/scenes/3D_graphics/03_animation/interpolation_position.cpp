...
void scene_model::setup_data(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    timer.t_max = keyframes[keyframes.size()-1].t-1;  // last time of the keyframe
    timer.t = timer.t_min;
...
void scene_model::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    const float t0 = (idx==0)? keyframes[keyframes.size() - 2].t - keyframes[keyframes.size() - 1].t  : keyframes[idx-1].t; // = t_{i-1}
    const float t1 = keyframes[idx  ].t; // = t_i
    const float t2 = keyframes[idx+1].t; // = t_{i+1}
    const float t3 = keyframes[idx+2].t;; // = t_{i+2}

    const vec3& p0 = (idx==0)? keyframes[keyframes.size() - 2].p : keyframes[idx-1].p; // = p_{i-1}
