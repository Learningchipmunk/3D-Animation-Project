...
void scene_model::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    for(particle_structure& particle : particles)
    {
        ...
        if(p.z <= -0.001f && p.x*p.x + p.y*p.y <= 4.0f)
        {
            const float theta = atan(sqrt(v.x*v.x + v.y*v.y)/ v.z);
            const float kt = 0.8f, kn = 0.7f;
            const vec3 vn = {0, 0, norm(v) * cos(theta)}, vt = norm(v) * sin(theta) * normalize(vec3{v.x, v.y, 0.0f});
            v = -kt*vt + kn*vn;
            p.z = 0.05f;
            p = p + dt * v;
